...
var words = ["VIT", "hello", "other"];
// var words = ["VIT", "hello", "what is", "the weather", "the time",
...
class Main {
    ...
    createPredictBtn() {
        ...
        predButton.addEventListener("mousedown", () => {
            ...
            if (Math.max(...exampleCount) > 0) {
                ...
                if (exampleCount[0] == 0) {
                    alert(`You haven't added examples for the wake word VIT`);
                    return;
                }
    ...
    createTrainingBtn() {
        ...
        trainButton.addEventListener("mousedown", () => {
            ...
            if (words.length == 3 && endWords.length == 1) {
                var proceed = confirm(
                    "You have not added any words.\n\nThe only query you can currently make is: 'VIT, hello'"
                );
...
class TextToSpeech {
    ...
    speak(word) {
        if (word == "VIT") {
            ...
            setTimeout(() => {
                // if no query detected after VIT is signed
        ...
        if (word != "VIT" && this.currentPredictedWords.length == 0) {
            console.log("first word should be VIT");
            console.log(word);
            return;
        }
